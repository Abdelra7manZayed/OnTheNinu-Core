...
CREATE TABLE supplier(
    ...
    pay_terms_days INT NOT NULL DEFAULT 0 CHECK (pay_terms_days >= 0),
    is_active BIT NOT NULL DEFAULT 1 CHECK (is_active IN (0, 1)),
    created_at DATETIME2(0) NOT NULL DEFAULT SYSDATETIME(),
    created_by INT NULL,
    updated_by INT NULL
);
...
CREATE TABLE employee(
    ...
    dep_id INT NULL,
    role_id INT NULL,
    salary DECIMAL(18,2) NULL CHECK (salary IS NULL OR salary >= 0),
...
CREATE TABLE register(
    ...
    register_code NVARCHAR(100) NOT NULL UNIQUE,
    is_active BIT NOT NULL DEFAULT 1 CHECK (is_active IN (0, 1)),
    created_at DATETIME2(0) NOT NULL DEFAULT SYSDATETIME(),
    created_by INT NULL,
    updated_by INT NULL,
...
CREATE TABLE product(
    ...
    stock_qty DECIMAL(18,2) NOT NULL DEFAULT 0 CHECK (stock_qty >= 0),
    reorder_level DECIMAL(18,2) NOT NULL DEFAULT 0 CHECK (reorder_level >= 0),
    is_active BIT NOT NULL DEFAULT 1 CHECK (is_active IN (0, 1)),
    created_at DATETIME2(0) NOT NULL DEFAULT SYSDATETIME(),
    created_by INT NULL,
    updated_by INT NULL,
...
CREATE TABLE inventory_movement(
    ...
    movement_type NVARCHAR(50) NOT NULL CHECK (movement_type IN ('opening_stock', 'purchase', 'sale', 'return', 'adjustment_in', 'adjustment_out')),
...
CREATE TABLE discount(
    ...
    type_d NVARCHAR(50) NOT NULL CHECK (type_d IN ('percentage', 'fixed_amount')),
    value_d DECIMAL(18,2) NOT NULL CHECK (value_d >= 0),
...
CREATE TABLE loyalty_account(
    ...
    card_number NVARCHAR(100) NOT NULL UNIQUE,
    points_balance INT NOT NULL DEFAULT 0 CHECK (points_balance >= 0),
    membership_level NVARCHAR(50) NOT NULL DEFAULT 'Basic' CHECK (membership_level IN ('Basic', 'Silver', 'Gold', 'Platinum')),
    created_at DATETIME2(0) NOT NULL DEFAULT SYSDATETIME(),
...
CREATE TABLE sale(
    ...
    sale_date DATETIME2(0) NOT NULL DEFAULT SYSDATETIME(),
...
CREATE TABLE sale_item(
    ...
    product_id INT NOT NULL,
    qty DECIMAL(18,2) NOT NULL CHECK (qty > 0),
...
CREATE TABLE payment(
    ...
    pay_date DATETIME2(0) NOT NULL DEFAULT SYSDATETIME(),
    status_p NVARCHAR(50) NOT NULL CHECK (status_p IN ('pending', 'paid', 'failed', 'refunded')),
    pay_method NVARCHAR(50) NOT NULL CHECK (pay_method IN ('cash', 'card', 'wallet', 'bank_transfer')),
    created_at DATETIME2(0) NOT NULL DEFAULT SYSDATETIME(),
    FOREIGN KEY (sale_id) REFERENCES sale(id)
);
GO
...
CREATE TABLE sale_return(
    ...
    sale_id INT NOT NULL,
    customer_id INT NULL,
    return_date DATETIME2(0) NOT NULL DEFAULT SYSDATETIME(),
    total_return DECIMAL(18,2) NOT NULL CHECK (total_return >= 0),
    reason NVARCHAR(MAX) NULL,
    created_at DATETIME2(0) NOT NULL DEFAULT SYSDATETIME(),
    FOREIGN KEY (sale_id) REFERENCES sale(id),
    FOREIGN KEY (customer_id) REFERENCES customer(id)
...
CREATE TABLE points_transaction(
    ...
    transaction_type NVARCHAR(50) NOT NULL CHECK (transaction_type IN ('earn', 'redeem', 'refund', 'adjustment')),
    transaction_date DATETIME2(0) NOT NULL DEFAULT SYSDATETIME(),
    created_at DATETIME2(0) NOT NULL DEFAULT SYSDATETIME(),
...
CREATE TABLE purchase_order(
    ...
    status_po NVARCHAR(50) NOT NULL CHECK (status_po IN ('pending', 'ordered', 'received', 'cancelled')),
    created_at DATETIME2(0) NOT NULL DEFAULT SYSDATETIME(),
    FOREIGN KEY (supplier_id) REFERENCES supplier(id),
    FOREIGN KEY (employee_id) REFERENCES employee(id)
);
...
CREATE TABLE shift(
    ...
    closing_balance DECIMAL(18,2) NULL CHECK (closing_balance IS NULL OR closing_balance >= 0),
    created_at DATETIME2(0) NOT NULL DEFAULT SYSDATETIME(),
    FOREIGN KEY (register_id) REFERENCES register(id),
    FOREIGN KEY (employee_id) REFERENCES employee(id)
);
...
CREATE UNIQUE INDEX UX_supplier_email_not_null ON supplier(email) WHERE email IS NOT NULL;
CREATE UNIQUE INDEX UX_employee_national_ID_not_null ON employee(national_ID) WHERE national_ID IS NOT NULL;
CREATE UNIQUE INDEX UX_employee_email_not_null ON employee(email) WHERE email IS NOT NULL;
CREATE UNIQUE INDEX UX_product_barcode_not_null ON product(barcode) WHERE barcode IS NOT NULL;
CREATE UNIQUE INDEX UX_customer_phone_not_null ON customer(phone) WHERE phone IS NOT NULL;
CREATE UNIQUE INDEX UX_customer_email_not_null ON customer(email) WHERE email IS NOT NULL;
GO
...
INSERT INTO user_account (id, employee_id, username, password_hash, is_active, last_login, created_at) VALUES
